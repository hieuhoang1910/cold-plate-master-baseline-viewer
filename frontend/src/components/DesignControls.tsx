  ...
  return (
    <div className="card">
      ...
      {gyroid ? (
        <>
          ...
          {pin ? (
            <>
              ...
              <Slider label="Pin diameter" unit="mm" min={Math.max(0.1, mode === 'explore' ? 0.1 : fl.t)} max={3.0} step={0.01} digits={2}
                value={design.pin_diameter_mm} onChange={(v) => onPatch({ pin_diameter_mm: v })}
                tiers={{ abs: rule.wallAbs, rec: rule.wallRec }} />
              <Slider label="Pin pitch" unit="mm" min={0.3} max={6.0} step={0.01} digits={2}
            ...
            <>
              ...
              <Slider label="Cell grading (jet-adaptive)" unit="" min={0} max={1.0} step={0.01} digits={2}
                value={design.cell_grading} onChange={(v) => onPatch({ cell_grading: v })} />
              <Slider label="Unit cell" unit="mm" min={0.5} max={6.0} step={0.05} digits={2}
                value={design.unit_cell_mm} onChange={(v) => onPatch({ unit_cell_mm: v })} />
              <Slider label="Wall thickness" unit="mm" min={fl.t} max={0.6} step={0.005} digits={3}
                value={design.wall_thickness_mm} onChange={(v) => onPatch({ wall_thickness_mm: v })}
                tiers={{ abs: rule.wallAbs, rec: rule.wallRec }} />
            </>
          )}

          <Slider label="Void fraction" unit="" min={0.15} max={0.9} step={0.005} digits={3}
            value={design.void_fraction} onChange={(v) => onPatch({ void_fraction: v })} />
          <Slider label="Surface area density" unit="m²/m³" min={1000} max={25000} step={50} digits={0}
            value={design.surface_area_density_m2_m3} onChange={(v) => onPatch({ surface_area_density_m2_m3: v })} />
          <Slider label="Hydraulic diameter" unit="mm" min={0.05} max={2.0} step={0.005} digits={3}
            value={design.hydraulic_diameter_mm} onChange={(v) => onPatch({ hydraulic_diameter_mm: v })}
            tiers={{ abs: rule.gapAbs, rec: rule.gapRec }} />
          <Slider label="Flow rate" unit="L/min" min={0.2} max={8.0} step={0.01} digits={2}
        ...
        <>
          {/* 2026-08-05 — ranges widened + steps refined (user request): drag at
              1 µm on widths, type exact values for anything finer. Every mesh
              we have measured now fits inside the slider range (Proto 1
          ...
          {!isStraight && (
            <>
              ...
              <Slider label="Wavelength λ" unit="mm" min={0.8} max={8.0} step={0.01} digits={2}
